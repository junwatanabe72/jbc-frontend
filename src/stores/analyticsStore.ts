...
export const useAnalyticsStore = create<AnalyticsState>((set, get) => ({
  ...
  generateReport: () => {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const totalUnits = 50;
      const occupiedUnits = Math.round(totalUnits * (0.8 + Math.random() * 0.2));
    ...
    // 申請分析データ
    const requestTypes = ['maintenance', 'construction', 'move_in_out', 'equipment', 'other'];
    const requestAnalytics: RequestAnalytics[] = requestTypes.map(type => {
      ...
      return {
        type: type === 'maintenance' ? 'メンテナンス' :
              type === 'construction' ? '工事' :
              type === 'move_in_out' ? '搬入・搬出' :
              type === 'equipment' ? '設備' : 'その他',
        count: typeRequests.length,
        avgProcessingTime: 3 + Math.random() * 7, // 3-10日
        approvalRate: typeRequests.length > 0 ? (approvedRequests.length / typeRequests.length) * 100 : 0,
      };
    });
    ...
    const currentOccupancy = occupancyData[occupancyData.length - 1]?.occupancyRate || 0;
    const previousOccupancy = occupancyData[occupancyData.length - 2]?.occupancyRate || 0;
    ...
    const analyticsData: AnalyticsData = {
      ...
      pendingRequests: requestStore.requests.filter(r => r.status === 'submitted' || r.status === 'under_review').length,
      avgProcessingTime: 5.2,
      
      bookingAnalytics,
      monthlyBookings: bookingAnalytics[bookingAnalytics.length - 1]?.totalBookings || 0,
      bookingUtilization: bookingAnalytics[bookingAnalytics.length - 1]?.utilizationRate || 0,
